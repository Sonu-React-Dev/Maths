# -*- coding: utf-8 -*-
"""
Phase 3 Ratio & Proportion Master Book Edition Generator
...
<head>
  ...
  <title>SSC CGL Maths — Phase 3: Ratio & Proportion (Complete Master Book Edition) | अनुपात एवं समानुपात</title>
  <meta name="description" content="SSC CGL Maths Phase 3: Ratio & Proportion complete textbook edition with 6 comprehensive theory chapters, 10 formula cards, 6 shortcuts, 6 traps, 20 basic questions, 20 mixed questions, 15 PYQs, interactive timed drill and 1-click print export.">
...
/* ── HERO ── */
.phase-hero {
  background: linear-gradient(135deg, rgba(246,194,68,0.15), rgba(0,229,255,0.12));
...
.phase-badge {
  ...
  background:linear-gradient(90deg,#f6c244,#00e5ff);
...
h1 {
  ...
  background:linear-gradient(90deg,#fff,#f6c244);
...
.hstat-val { font-weight:800; color:var(--gold); font-size:1.1rem; }
...
.chap-badge {
  display:inline-block; background:rgba(246,194,68,0.1); color:var(--gold);
  font-size:0.72rem; font-weight:700; padding:3px 10px; border-radius:6px; margin-bottom:8px;
  border:1px solid rgba(246,194,68,0.2);
}
.chap-title {
...
.book-list li::before {
  content:'✦'; color:var(--gold); font-size:0.8rem; flex-shrink:0; margin-top:3px;
}
.concept-callout {
  background: rgba(246,194,68,0.04);
...
.book-table th {
  background: var(--surface-elevated); color:var(--gold);
...
.book-table tr:hover td { background:rgba(246,194,68,0.02); }
...
.day-label {
  background:linear-gradient(135deg,rgba(246,194,68,0.15),rgba(0,229,255,0.15));
  border:1px solid var(--border); border-radius:8px;
  text-align:center; padding:10px 6px;
}
.day-tag { font-size:0.75rem; font-weight:700; color:var(--gold); }
.day-name { font-size:0.82rem; font-weight:600; color:#fff; margin:2px 0; }
.day-drill { font-size:0.68rem; color:var(--primary); }
...
.day-topics li::before { content:'→'; color:var(--gold); flex-shrink:0; margin-top:1px; }

/* ── FORMULA VAULT ── */
.formula-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(330px,1fr)); gap:12px; margin-bottom:22px; }
.formula-card {
  background:var(--surface); border:1px solid rgba(246,194,68,0.15);
  border-radius:12px; overflow:hidden;
}
.fc-header {
  background:rgba(246,194,68,0.06); border-bottom:1px solid rgba(246,194,68,0.15);
  padding:10px 14px; display:flex; align-items:center; justify-content:space-between;
}
.fc-name { font-weight:700; color:var(--gold); font-size:0.88rem; }
.copy-btn {
  background:var(--surface-elevated); color:var(--text-muted);
...
.fc-formula {
  background:var(--surface-elevated); border-left:3px solid var(--gold);
  padding:10px 12px; border-radius:6px; margin-bottom:8px;
  font-family:'Fira Code',monospace; font-size:0.86rem; color:#fde68a; word-break:break-all;
}
...
.q-block:hover { border-color: rgba(246,194,68,0.3); }
.q-head {
...
.q-num {
  ...
  background: rgba(246,194,68,0.1); padding: 2px 10px; border-radius: 6px;
}
.q-tag {
  font-size: 0.72rem; color: var(--primary);
  background: rgba(0,229,255,0.1); padding: 2px 8px; border-radius: 4px;
}
.q-text {
...
.btn-hint { background: rgba(0,229,255,0.1); color: var(--primary); border-color: rgba(0,229,255,0.3); }
.btn-hint:hover { background: rgba(0,229,255,0.2); }
...
.hint-content { background: rgba(0,229,255,0.06); border-left: 3px solid var(--primary); color: #bae6fd; }
...
.rate-btn:hover { border-color: var(--gold); color: #fff; }
...
.timer-display {
  ...
  text-shadow:0 0 20px rgba(246,194,68,0.4);
}
...
.print-btn-primary {
  background:var(--gold); color:#080b12; font-weight:700;
...
def get_topbar_and_hero():
  ...
  <header class="phase-hero">
    <span class="phase-badge">Phase 3 • Days 21–26 • Complete Book Edition</span>
    <h1>Phase 3: Ratio &amp; Proportion (अनुपात एवं समानुपात)</h1>
    <p class="hero-hindi">Bridging Term, Proportionals, Componendo &amp; Dividendo, Cross-Multiplication &amp; Word Problems</p>
  ...
  <div style="background:rgba(246,194,68,0.04);border:1px solid rgba(246,194,68,0.15);border-radius:12px;padding:12px 16px;margin-bottom:20px;font-size:0.82rem;color:#7a90a8;">
    <strong style="color:var(--gold);">⏱️ AI Tutor Standard Lesson Protocol:</strong>
    &nbsp;Ratio Basics (5–10m) &rarr; Bridging / N-Method (5m) &rarr; Proportional Formulas (5m) &rarr; Speed Shortcuts + Traps (5–10m) &rarr; Foundation Practice (20 Qs) &rarr; Mixed Practice (20 Qs) &rarr; Real PYQ Lab (15 Qs) &rarr; Error Log &rarr; Active Recall.
    <br><strong style="color:var(--primary);">Rule:</strong> Question ko pehle rough sheet par solve karein &mdash; turant solution mat dekhein!
  </div>
'''
...
def get_day_plan():
    return r'''
  <!-- DAY PLAN -->
  <h2 class="sec-title" id="sec-plan">📅 Days 21–26 (6 Days) — Day-by-Day Master Syllabus Plan</h2>
  <div class="day-cards">
    <div class="day-card">
      <div class="day-label">
        <div class="day-tag">Day 21</div>
        <div class="day-name">Ratio Basics &amp; Combining Methods</div>
        <div class="day-drill">📝 25 Speed Drills</div>
      </div>
      <div>
        <div style="font-weight:700;color:#fff;font-size:0.92rem;margin-bottom:6px;">Fractional Ratios, Bridging Terms &amp; The Inverted-N Technique</div>
        <ul class="day-topics">
          <li><strong>Definition:</strong> Ratio is a dimensionless comparison $a : b = a/b$. Multiplying/dividing both terms by the same non-zero number leaves the ratio unchanged.</li>
          <li><strong>Fractional Ratio Simplification:</strong> $\frac{1}{2} : \frac{2}{3} : \frac{3}{4} \implies$ multiply by $\text{LCM}(2, 3, 4) = 12 \implies 6 : 8 : 9$.</li>
          <li><strong>Combining $A:B$ and $B:C$:</strong> Equalize the bridging term $B$. Inverted-N method: $A \times B_1, B_1 \times B_2, B_2 \times C$.</li>
          <li><strong>4-Term Combining ($A:B:C:D$):</strong> Align columns and fill vacant slots horizontally with adjacent values.</li>
        </ul>
      </div>
    </div>
    ...
    <div class="day-card">
      <div class="day-label">
        <div class="day-tag">Day 22</div>
        <div class="day-name">Proportion &amp; Algebraic Properties</div>
        <div class="day-drill">📝 20 Formula Drills</div>
      </div>
      <div>
        <div style="font-weight:700;color:#fff;font-size:0.92rem;margin-bottom:6px;">Extremes &amp; Means, Componendo &amp; Dividendo (C&amp;D) Mastery</div>
        <ul class="day-topics">
          <li><strong>Fundamental Proportion Law:</strong> $a : b :: c : d \implies a \times d = b \times c$ ($\text{Product of Extremes} = \text{Product of Means}$).</li>
          <li><strong>Componendo &amp; Dividendo:</strong> If $\frac{a}{b} = \frac{c}{d}$, then $\frac{a+b}{a-b} = \frac{c+d}{c-d}$. Essential for solving expressions like $\frac{\sqrt{x+1}+\sqrt{x-1}}{\sqrt{x+1}-\sqrt{x-1}} = 3$.</li>
          <li><strong>Addendo Theorem:</strong> If $\frac{a}{b} = \frac{c}{d} = \frac{e}{f} = k$, then $\frac{a + c + e}{b + d + f} = k$.</li>
        </ul>
      </div>
    </div>
    ...
    <div class="day-card">
      <div class="day-label">
        <div class="day-tag">Day 23</div>
        <div class="day-name">Compound, Duplicate &amp; Inverse Ratios</div>
        <div class="day-drill">📝 20 Terminology Qs</div>
      </div>
      <div>
        <div style="font-weight:700;color:#fff;font-size:0.92rem;margin-bottom:6px;">All Classical Ratio Types &amp; Inversion Engine</div>
        <ul class="day-topics">
          <li><strong>Duplicate Ratio:</strong> $a^2 : b^2$. Sub-duplicate: $\sqrt{a} : \sqrt{b}$.</li>
          <li><strong>Triplicate Ratio:</strong> $a^3 : b^3$. Sub-triplicate: $\sqrt[3]{a} : \sqrt[3]{b}$.</li>
          <li><strong>Reciprocal / Inverse Ratio:</strong> For $a : b \implies b : a$. For $a : b : c \implies \frac{1}{a} : \frac{1}{b} : \frac{1}{c} = bc : ac : ab$.</li>
          <li><strong>Compound Ratio:</strong> For $a:b$ and $c:d \implies (ac) : (bd)$.</li>
        </ul>
      </div>
    </div>
    ...
    <div class="day-card">
      <div class="day-label">
        <div class="day-tag">Day 24</div>
        <div class="day-name">Mean, Third &amp; Fourth Proportional</div>
        <div class="day-drill">📝 25 Formula Drills</div>
      </div>
      <div>
        <div style="font-weight:700;color:#fff;font-size:0.92rem;margin-bottom:6px;">Calculations of Proportional Terms &amp; The Added/Subtracted Number $x$</div>
        <ul class="day-topics">
          <li><strong>Mean Proportional of $a$ and $b$:</strong> $x = \sqrt{ab}$.</li>
          <li><strong>Third Proportional to $a$ and $b$:</strong> $x = \frac{b^2}{a}$.</li>
          <li><strong>Fourth Proportional to $a, b, c$:</strong> $x = \frac{bc}{a}$.</li>
          <li><strong>Number $x$ Subtracted/Added:</strong> What number $x$ must be subtracted from $a, b, c, d$ so the remainders are proportional?
    ...
    <div class="day-card">
      <div class="day-label">
        <div class="day-tag">Day 25</div>
        <div class="day-name">Ages, Coins &amp; Cross-Multiplication</div>
        <div class="day-drill">📝 25 Word Problems</div>
      </div>
      <div>
        <div style="font-weight:700;color:#fff;font-size:0.92rem;margin-bottom:6px;">Income-Expenditure Cross Method &amp; Constant Difference in Age Systems</div>
        <ul class="day-topics">
          <li><strong>Cross-Multiplication Method:</strong> For $I_1 : I_2$ and $E_1 : E_2$ with savings $S_1, S_2$. Cross product difference equals net product difference!</li>
          <li><strong>Age Invariance Rule:</strong> Difference between two people's ages is invariant across any number of years! Equalize the gap in ratio units.</li>
          <li><strong>Coin Box Problems:</strong> Convert ratio of coins to ratio of monetary values by multiplying with face value (₹1 &rarr; $\times 1$, 50p &rarr; $\times 1/2$, 25p &rarr; $\times 1/4$).</li>
        </ul>
      </div>
    </div>
    ...
    <div class="day-card">
      <div class="day-label">
        <div class="day-tag">Day 26</div>
        <div class="day-name">Mixed PYQ Lab &amp; Phase 3 Mastery Test</div>
        <div class="day-drill">⏱️ 25 Questions Timed Drill</div>
      </div>
      <div>
        <div style="font-weight:700;color:#fff;font-size:0.92rem;margin-bottom:6px;">High-Speed Tier-1 &amp; Tier-2 Cross Applications</div>
        <ul class="day-topics">
          <li><strong>Mastery Criteria:</strong> Multi-term combining, fractional ratio distribution errors, and coin problems solved in &le;40s each.</li>
          <li><strong>Target:</strong> &ge;85% accuracy in 25-question test under 22 minutes.</li>
        </ul>
      </div>
    </div>
  </div>
